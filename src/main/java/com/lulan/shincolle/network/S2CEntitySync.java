...
public class S2CEntitySync implements IMessage {
    ...
    }    //必須要有空參數constructor, forge才能使用此class
    ...
    public void fromBytes(ByteBuf buf) {
        ...
        switch (this.packetType) {
            ...
            case PID.SyncShip_Attrs:    //sync all attrs
                ...
                if (bonus) {
                    this.valueByte1 = PacketHelper.readByteArray(buf);
                } else {
                    this.valueByte1 = null;
                }
                if (raw) {
                    this.valueFloat1 = PacketHelper.readFloatArray(buf);
                } else {
                    this.valueFloat1 = null;
                }
                if (equip) {
                    this.valueFloat2 = PacketHelper.readFloatArray(buf);
                } else {
                    this.valueFloat2 = null;
                }
                if (morale) {
                    this.valueFloat3 = PacketHelper.readFloatArray(buf);
                } else {
                    this.valueFloat3 = null;
                }
                if (potion) {
                    this.valueFloat4 = PacketHelper.readFloatArray(buf);
                } else {
                    this.valueFloat4 = null;
                }
                if (formation) {
                    data = PacketHelper.readFloatArray(buf);
                    this.valueFloat5 = new float[Attrs.AttrsLength + 1];
                    //copy data
                    System.arraycopy(data, 0, this.valueFloat5, 0, Attrs.AttrsLength);
                    //read a float
                ...
                if (buffed) {
                    ...
                    //copy data
                    System.arraycopy(data, 0, this.valueFloat6, 0, Attrs.AttrsLength);
                    //read a float
