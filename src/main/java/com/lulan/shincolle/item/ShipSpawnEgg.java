...
public class ShipSpawnEgg extends BasicItem {
    ...
    private static final List<Integer> shipList = new ArrayList<Integer>();
    ...
    private void initEntityAttribute(ItemStack stack, EntityPlayer player, BasicEntityShip entity) {
        ...
        if (stack.getItemDamage() > 1) {
            ...
            if (nbt != null) {
                ...
                try {
                    if (flags.length >= 16) {
                        entity.setStateFlag(ID.F.IsMarried, flags[0] > 0);
                        entity.setStateFlag(ID.F.UseMelee, flags[1] > 0);
                        entity.setStateFlag(ID.F.UseAmmoLight, flags[2] > 0);
                        entity.setStateFlag(ID.F.UseAmmoHeavy, flags[3] > 0);
                        entity.setStateFlag(ID.F.UseAirLight, flags[4] > 0);
                        entity.setStateFlag(ID.F.UseAirHeavy, flags[5] > 0);
                        entity.setStateFlag(ID.F.UseRingEffect, flags[6] > 0);
                        entity.setStateFlag(ID.F.OnSightChase, flags[7] > 0);
                        entity.setStateFlag(ID.F.PVPFirst, flags[8] > 0);
                        entity.setStateFlag(ID.F.AntiAir, flags[9] > 0);
                        entity.setStateFlag(ID.F.AntiSS, flags[10] > 0);
                        entity.setStateFlag(ID.F.PassiveAI, flags[11] > 0);
                        entity.setStateFlag(ID.F.TimeKeeper, flags[12] > 0);
                        entity.setStateFlag(ID.F.PickItem, flags[13] > 0);
                        entity.setStateFlag(ID.F.ShowHeldItem, flags[14] > 0);
                        entity.setStateFlag(ID.F.AutoPump, flags[15] > 0);
                    }
