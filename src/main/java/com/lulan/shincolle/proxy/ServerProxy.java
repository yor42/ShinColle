...
public class ServerProxy extends CommonProxy {
     ...
     * <player UUID: int, player target: map<target class name hash, target class
     * name>>
     * <p>
     * use:
     ...
     * 3. get: player login event (new player), ship entity construction (new ship),
     * check team
     * 4. set: player login event (new player), ship entity construction (new ship),
     * create team
     */
    ...
    // init server proxy variables, called when world loaded and initServerFile =
    // false
    public static void initServerProxy(World world) {
        ...
        if (serverData != null && ShinWorldData.nbtData != null) {
            ...
            nextPlayerID = ShinWorldData.nbtData.getInteger(ShinWorldData.TAG_NEXTPLAYERID);
            nextShipID = ShinWorldData.nbtData.getInteger(ShinWorldData.TAG_NEXTSHIPID);

            // load unattackable list
            NBTTagList unatktag = ShinWorldData.nbtData.getTagList(UNATK_TARGET_CLASS, Constants.NBT.TAG_STRING);
            ...
            NBTTagList list = ShinWorldData.nbtData.getTagList(ShinWorldData.TAG_PLAYERDATA, Constants.NBT.TAG_COMPOUND);
            ...
            NBTTagList list2 = ShinWorldData.nbtData.getTagList(ShinWorldData.TAG_TEAMDATA, Constants.NBT.TAG_COMPOUND);
            ...
            NBTTagList list3 = ShinWorldData.nbtData.getTagList(ShinWorldData.TAG_SHIPDATA, Constants.NBT.TAG_COMPOUND);
            ...
            for (int i = 0; i < list3.tagCount(); i++) {
                ...
                CacheDataShip sData = new CacheDataShip(eid, wid, cid, isDead, pos[0], pos[1],
                        pos[2], sTag);
    ...
    /**
     * unattackable target list, return true = add target, false = remove target or
     * do nothing
     */
    // add
    ...
    public static HashMap<Integer, String> getPlayerTargetClass(int pid) {
        if (pid > 0)
            return customTagetClass.get(pid);
        return null;
    }
    ...
    public static void updatePlayerID(EntityPlayer player) {
        LogHelper.debug("DEBUG: update player: " + player.getDisplayNameString() + " " + player.getUniqueID() + " "
                + player.getEntityId());
    ...
    public static void updateShipID(BasicEntityShip ship) {
        ...
        // update ship data
        if (uid > 0) {
            LogHelper.debug("DEBUG: update ship: update ship id " + uid + " eid: " + ship.getEntityId() + " world: "
                    + ship.world.provider.getDimension());
        ...
        // ship id < 0, create one
            ...
            // set init value
            if (uid <= 0)
                uid = 100; // ship id init value = 100

            LogHelper.debug("DEBUG: update ship: create sid: " + uid + " eid: " + ship.getEntityId() + " world: "
                    + ship.world.provider.getDimension());
            ship.setShipUID(uid);
            CacheDataShip sdata = new CacheDataShip(ship.getEntityId(), ship.world.provider.getDimension(),
                    ship.getShipClass(), ship.isDead, ship.posX, ship.posY, ship.posZ,
            ...
            if (serverTicks % updateRadarTicks == 0) {
                ...
                if (needUpdate) {
                    ...
                    shipListByPlayer.forEach((pid, value) -> {
                        ...
                        if (player != null) {
                            // sync data to player
                            CommonProxy.channelG.sendTo(new S2CGUIPackets(S2CGUIPackets.PID.SyncGUI_ShipList, value),
                                    player);
                        }
                    });
