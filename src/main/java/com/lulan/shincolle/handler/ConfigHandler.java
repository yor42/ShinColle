...
public class ConfigHandler {
    ...
    private static void loadConfiguration() {
        ...
        // 是否顯示custom name tag
        alwaysShowTeamParticle = config.getBoolean("AlwaysShow_TeamCircle", CATE_GENERAL, false,
                "Always show team circle indicator particle");

        // boss生成cd設定 (ticks)
        bossCooldown = config.getInt("Cooldown_Boss", CATE_GENERAL, 4800, 20, 1728000, "Boss spawn cooldown");

        // 玩家離開多遠時關閉GUI
        closeGUIDist = config.getInt("Close_GUI_Distance", CATE_GENERAL, 64, 2, 64,
                "Close inventory GUI if ship away from player X blocks");

        // chuknk loader使用模式
        chunkloaderMode = config.getInt("Mode_ChunkLoader", CATE_GENERAL, 2, 0, 2,
                "Chunk loader mode: 0: disable, 1: only 1 chunk each ship, 2: 3x3 chunks each ship");
        ...
        // boss刪除時間 (ticks)
        despawnBoss = config.getInt("Despawn_Boss", CATE_GENERAL, 12000, -1, 1728000,
                "Despawn time of boss ship , -1 = do NOT despawn");

        // 雜魚刪除時間 (ticks)
        despawnMinion = config.getInt("Despawn_Minion", CATE_GENERAL, 600, -1, 1728000,
                "Despawn time of nonboss ship, -1 = do NOT despawn");

        // 野生生怪蛋刪除時間 (ticks)
        despawnEgg = config.getInt("Despawn_Egg", CATE_GENERAL, 12000, -1, 1728000,
                "Despawn time of spawn egg of ship mob, -1 = do NOT despawn");

        // grudge掉落率設定
        dropGrudge = config.getFloat("DropRate_Grudge", CATE_GENERAL, 1F, 0F, 64F,
                "Grudge drop rate (ex: 0.5 = 50% drop 1 grudge, 5.5 = drop 5 grudge + 50% drop 1 grudge)");

        // 是否開啟簡單模式
        easyMode = config.getBoolean("Mode_Easy", CATE_GENERAL, false,
                "Easy mode: decrease Large Construction resources requirement, increase ammo / grudge gained from items");

        // 是否開啟誤傷模式
        friendlyFire = config.getBoolean("Friendly_Fire", CATE_GENERAL, true,
                "false: disable damage done by player (except owner)");

        // 解體獲得材料量設定, mob drop類ship限定
        kaitaiAmountSmall = config.getInt("Recycle_Small", CATE_GENERAL, 20, 0, 1000,
                "Recycle amount by Dismantle Hammer for copmmon ship, ex: Ro500.");
        kaitaiAmountLarge = config.getInt("Recycle_Large", CATE_GENERAL, 20, 0, 1000,
                "Recycle amount by Dismantle Hammer for rare ship, ex: Yamato.");

        // 野生船艦是否主動攻擊玩家
        mobAttackPlayer = config.getBoolean("Attack_Player_ShipMob", CATE_GENERAL, true,
                "for mob ship, true: attack player automatically");

        // 是否默认攻击原版敌对生物
        mcMobIFF = config.getBoolean("MC_HostileMob_IFF", CATE_GENERAL, true,
                "for mob ship, true: put mc hostile mobs into iff list");

        // 船艦是否主動攻擊玩家
        shipAttackPlayer = config.getInt("Attack_Player_Ship", CATE_GENERAL, 0, 0, 3,
                "for pet ship, 0: ship don't attack player automatically, 1: attack hostile player, 2: attack hostile and neutral player, 3: attack all player even if the player isn't in a team");

        // 是否把多金屬當成錳礦
        polyAsMn = config.getBoolean("Polymetal_as_Mn", CATE_GENERAL, false,
                "true: Polymetallic Nodules = Manganese Dust, Polymetallic Ore = Manganese Ore");

        // desk雷達更新間隔
        radarUpdate = config.getInt("Radar_Update", CATE_GENERAL, 64, 20, 6000,
                "Radar update interval (ticks) in Admiral's Desk GUI");

        // 是否顯示custom name tag
        showTag = config.getBoolean("NameTag_AlwaysShow", CATE_GENERAL, true, "Always show custom name tag");

        // name tag顯示距離
        nameTagDist = config.getInt("NameTag_Distance", CATE_GENERAL, 16, 1, 64,
                "Show name tag if player get close to ship X blocks");

        // ship列表顯示數量
        shipNumPerPage = config.getInt("Command_ShipNum", CATE_GENERAL, 5, 1, 5000,
                "#Ship per page for command: /ship list");
        ...
        // 畫深海熱漩是否開啟景深
        vortexDepth = config.getBoolean("Depth_HadalVortex", CATE_GENERAL, false,
                "Enable depth while rendering Hadal Vortex block.");

        // 野生艦隊生成數量
        spawnBossNum = config.getInt("Spawn_Boss_Number", CATE_GENERAL, 2, 1, 10,
                "large hostile ship (boss) number per spawn");
        ...
        pairDistChest = config.getInt("PairingDist_Chest", CATE_GENERAL, 16, 0, 64,
                "Max pairing distance between waypoint and chest");
        pairDistWp = config.getInt("PairingDist_Waypoint", CATE_GENERAL, 48, 0, 64,
                "Max pairing distance between waypoints");

        // buff debuff TODO not configurable for now
        // buffSaturation = config.getInt("Saturation", CATE_BUFF, 20, 0, 5000, "add X
        // grudge value to ship every 32 ticks");

        // 是否開啟林業支援
        enableIC2 = config.getBoolean("Mod_IC2", CATE_INTERMOD, true,
                "Enable IC2 module if mod existed: add EU related function.");
        propDrumEU = config.get(CATE_INTERMOD, "Drum_EU", drumEU,
        ...
        enableMetamorphSkill = config.getBoolean("Mod_MetamorphSkill", CATE_INTERMOD, true,
                "Enable Metamorph module, if true: 1. player can use ship skill in morphing, 2. if no grudge, player will be demorphed.");
        expGainPlayerSkill = config.getFloat("Metamorph_ExpGain", CATE_INTERMOD, 6F, 0F, 1000F,
                "Exp modify for casting ship attack skill by player in morph, final exp = raw exp * Metamorph_ExpGain, req: Metamorph mod.");
        morphHPRatio = config.getFloat("Metamorph_HPRatio", CATE_INTERMOD, 0.1F, 0F, 10F,
                "HP modify of player in morph, final HP = 20 + shipHP * Metamorph_HPRatio, req: Metamorph mod.");
        morphDmgTakenRatio = config.getFloat("Metamorph_DmgTakenRatio", CATE_INTERMOD, 0.2F, 0F, 1F,
                "Damage by ship attack modify of player in morph, final DamageTaken = raw damage * Metamorph_DmgTakenRatio, req: Metamorph mod.");

        // 讀取 ship setting設定
        canFlare = config.getBoolean("Can_Flare", CATE_SHIP, true,
                "Can ship spawn Flare lighting effect, CLIENT SIDE only");
        canSearchlight = config.getBoolean("Can_Searchlight", CATE_SHIP, true,
                "Can ship spawn Searchlight lighting effect, CLIENT SIDE only");
        canTeleport = config.getBoolean("Can_Teleport", CATE_SHIP, true,
                "Can ship teleport to owner/guarding position if too far away. NOTE: set false if ship usually disappear/despawn after teleport!");
        checkRing = config.getBoolean("Check_Ring", CATE_SHIP, true,
                "Should check wedding ring when spawning NON-BOSS ship mob");
        timeKeeping = config.getBoolean("Can_Timekeeping", CATE_SHIP, true,
                "Play timekeeping sound every 1000 ticks (1 minecraft hour)");
        ...
        baseCaressMorale = config.getInt("Caress_BaseMorale", CATE_SHIP, 20, 1, 5000,
                "base morale value per CaressTick (4 ticks)");
        modernLimit = config.getInt("Attrs_Limit_Modernization", CATE_SHIP, 3, 3, 100,
                "Max upgrade level by Modernization Toolkit");
        searchlightCD = config.getInt("CD_SearchLight", CATE_SHIP, 4, 1, 256,
                "Cooldown for placing light block of searchlight");
        airplaneDelay = config.getInt("CD_AirplaneRecovery", CATE_SHIP, 3600, 1, 30000,
                "Base cooldown for airplane recovery, actual recovery time = CD_AirplaneRecovery / attack speed + 20");

        // array data
        propShip = config.get(CATE_SHIP, "Attrs_Scale", scaleShip,
        ...
        propAtkDly = config.get(CATE_SHIP, "Attack_Fixed_Delay", fixedAttackDelay,
        ...
        propHeldItem = config.get(CATE_SHIP, "Held_Item", scaleHeldItem,
                "Ship held item scaling: scale, offset X, offset Y, offset Z");
        propDrumLiquid = config.get(CATE_SHIP, "Drum_Liquid", drumLiquid,
        ...
        propShipTeleport = config.get("ship setting", "ship_teleport", shipTeleport,
        ...
        propTask = config.get(CATE_SHIP, "Task_Enable", enableTask,
                "set true to enable the task: cooking, fishing, mining, crafting");

        propShipyardSmall = config.get(CATE_GENERAL, "Tile_SmallShipyard", tileShipyardSmall,
                "Small shipyard: max fuel storage, build speed, fuel magnification");
        propShipyardLarge = config.get(CATE_GENERAL, "Tile_LargeShipyard", tileShipyardLarge,
        ...
        propPosHUD = config.get(CATE_GENERAL, "Position_HUD", posHUD,
                "HUD position of mounts skills: x, y (0.5D = middle of window)");

        // ship vs ship damage modifier
        dmgSvS = config.getInt("DmgTaken_SvS", CATE_SHIP, 100, 0, 10000,
                "Ship vs Ship damage modifier, 20 = damage * 20% ");
        expMod = config.getInt("EXP_Modifier", CATE_SHIP, 20, 1, 10000,
                "ship experience modifier, 20 = level 150: 150*20+20 = 3020");

        // WORLD GEN
        polyOreBaseRate = config.getInt("Polymetal_Ore", CATE_WORLD, 7, 0, 100, "Polymetallic Ore clusters in one chunk");
        polyGravelBaseRate = config.getInt("Polymetal_Gravel", CATE_WORLD, 4, 0, 100,
                "Polymetallic Gravel clusters in one chunk");
        propPolyGravel = config.get(CATE_WORLD, "Polymetal_Gravel_Replace", polyGravelBaseBlock,
                "PolyGravel replaced block: stone, gravel, sand, dirt", true, 4);
