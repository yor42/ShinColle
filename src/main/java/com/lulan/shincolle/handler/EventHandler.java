...
//	}
    ...
    public void onEntityAttack(LivingAttackEvent event) {
        ...
        Entity attacker = event.getSource().getTrueSource();                //attacker or summons
        Entity attackerSource = event.getSource().getImmediateSource();    //attacker or summoner
...
//				(event.player.isSneaking() ? 1 : 0));
            ...
            if ((event.player.ticksExisted & 15) == 0) {
                ...
                ItemStack itemRing = ItemStack.EMPTY;

                //get ring itemstack
                for (int i = 0; i < event.player.inventory.getSizeInventory(); ++i) {
                    if (!event.player.inventory.getStackInSlot(i).isEmpty() &&
                ...
                if (capa != null) {
                    ...
                    if (!itemRing.isEmpty() && itemRing.hasTagCompound()) {
    ...
    public void onAnvilUpdate(AnvilUpdateEvent event) {
        ...
        if (!equip.isEmpty() && !book.isEmpty() &&
            ...
            for (Enchantment ench : bookench.keySet()) {
                if (ench != null) {
                    //檢查equip跟book是否有相同附魔
                    int lv1 = equipench.containsKey(ench) ? equipench.get(ench).intValue() : 0;
                    int lv2 = bookench.get(ench).intValue();
