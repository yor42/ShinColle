...
abstract public class BasicEntityShipCV extends BasicEntityShip implements IShipAircraftAttack {
    ...
    public void setNumAircraftLight(int par1) {
        if (this.world.isRemote) {    //client端沒有max值可以判定, 因此直接設定即可
    ...
    public void onLivingUpdate() {
        ...
        if (!this.world.isRemote) {
            ...
            if (this.delayAircraft <= 0) {
                ...
                if (delayAircraft > ConfigHandler.airplaneDelay)
                    delayAircraft = ConfigHandler.airplaneDelay;    //fix: spd還沒設完值就除 會導致delay變超大 (除以0)
