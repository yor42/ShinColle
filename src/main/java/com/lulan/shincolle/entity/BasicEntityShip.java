...
public abstract class BasicEntityShip extends EntityTameable implements IShipCannonAttack, IShipGuardian, IShipFloating, IShipCustomTexture, IShipMorph {
    ...
    //basic attribute
    protected static final IAttribute MAX_HP = (new RangedAttribute(null, "generic.maxHealth", 4D, 0D, 30000D)).setDescription("Max Health").setShouldWatch(true);
    //misc flags
    ...
    /**
     * Update Flag, index by ID.UpdateFlag
     */
    ...
    protected void playTimeSound(int hour) {
        SoundEvent sound = getCustomSound(hour + 10, this);
    ...
    public void sendSyncPacket(byte type, boolean sendAll) {
        if (!world.isRemote) {
            ...
            if (sendAll) {
                ...
                //owner在附近才需要sync
                if (player != null && this.getDistanceSq(player) <= 64F) {
        ...
        if (!this.world.isRemote) {
            ...
            if (TeamHelper.checkSameOwner(this, player)) {
                ...
                if (player.isSneaking()) {
                    ...
                    if (!EntityHelper.getPointerInUse(player).isEmpty()) {
                    ...
                    if (this.ticksExisted % 32 == 0) {
                        if (!this.isMorph) {
                            ...
                            if (player != null && player.dimension == this.getGuardedPos(3)) {
                                ...
                                if (!item.isEmpty() && item.getItemDamage() < 3 || ConfigHandler.alwaysShowTeamParticle) {
...
//        			+" "+form[idd]+" "+buf[idd]);
    ...
    public void decrGrudgeNum(int value) {
        ...
        setStateFlag(ID.F.NoFuel, StateMinor[ID.M.NumGrudge] <= 0);
    ...
    protected int findItemInSlot(ItemStack parItem, boolean noMeta) {
        ...
        for (int i = ContainerShipInventory.SLOTS_SHIPINV; i < CapaShipInventory.SlotMax; i++) {
            ...
            if (!slotitem.isEmpty() && slotitem.getItem().equals(parItem.getItem())) {
                if (noMeta) {
                    return i;    //found item
                } else {
    ...
    protected void updateConsumeItem() {
        ...
        if (!this.hasAmmoLight()) {
            this.decrAmmoNum(0, 0);
        }
        if (!this.hasAmmoHeavy()) {
            this.decrAmmoNum(1, 0);
        }
    ...
    protected void updateMorale() {
        ...
        if (EntityHelper.checkShipOutOfCombat(this)) {
            if (m < ID.Morale.L_Normal + 1000) {    //take 5 min from 0 to 2100
        ...
        switch (EntityHelper.getMoraleLevel(m)) {
            ...
            case 2:   //normal
                ...
                if (body == getSensitiveBody()) {
                    ...
                    if (ran.nextInt(6) == 0) {
                        this.pushAITarget();
                        this.playSound(getCustomSound(5, this), this.getSoundVolume(), 1F / (this.getRNG().nextFloat() * 0.2F + 0.9F));
                    }
                }
                //other reaction
                else {
...
//  		}
    ...
    public boolean canShowHeldItem() {
        return this.getStateFlag(ID.F.ShowHeldItem) && this.getAttackTick() <= 0 &&
