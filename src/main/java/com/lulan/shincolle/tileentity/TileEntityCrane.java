...
public class TileEntityCrane extends BasicTileInventory implements ITileWaypoint, ITickable {
    ...
    private boolean isInventoryFull() {
        ...
        if (this.enabLoad && this.ship != null)
            fullList[0] = InventoryHelper.checkInventoryFull(this.ship.getCapaShipInventory());
        else fullList[0] = true;

        //unloading item: check chest full
        if (this.enabUnload && this.chest != null) fullList[1] = InventoryHelper.checkInventoryFull(this.chest);
        ...
        if (this.modeLiquid == 1 && this.tank != null) {
            ...
            if (this.ship != null)
                fullList[2] = InventoryHelper.checkFluidContainer(this.ship.getCapaShipInventory(), fs, true);
        ...
        if (this.modeLiquid == 2 && this.tank != null) {
            FluidStack fs = this.tank.getFluid() == null ? null : this.tank.getFluid().copy();
            if (this.chest != null) fullList[3] = InventoryHelper.checkFluidContainer(this.chest, fs, true);
    ...
    private boolean isInventoryEmpty() {
        ...
        if (this.enabLoad && this.chest != null)
            emptyList[0] = InventoryHelper.checkInventoryEmpty(this.chest, this.getItemstackTemp(true), this.getItemMode(true), checkMetadata, checkNbt, checkOredict);
        else emptyList[0] = true;

        //unloading item: check ship empty
        if (this.enabUnload && this.ship != null)
            emptyList[1] = InventoryHelper.checkInventoryEmpty(this.ship.getCapaShipInventory(), this.getItemstackTemp(false), this.getItemMode(false), checkMetadata, checkNbt, checkOredict);
        else emptyList[1] = true;

        //loading fluid: check chest empty
        if (this.modeLiquid == 1 && this.tank != null) {
            FluidStack fs = this.tank.getFluid() == null ? null : this.tank.getFluid().copy();
            if (this.chest != null) emptyList[2] = InventoryHelper.checkFluidContainer(this.chest, fs, false);
        ...
        if (this.modeLiquid == 2 && this.tank != null) {
            FluidStack fs = this.tank.getFluid() == null ? null : this.tank.getFluid().copy();
            if (this.ship != null)
                emptyList[3] = InventoryHelper.checkFluidContainer(this.ship.getCapaShipInventory(), fs, false);
    ...
    private boolean isInventoryExcess() {
        ...
        if (this.enabLoad && this.ship != null)
            excessList[0] = InventoryHelper.checkInventoryAmount(this.ship.getCapaShipInventory(), this.getItemstackTemp(true), this.getItemMode(true), checkMetadata, checkNbt, checkOredict, true);
        else excessList[0] = true;

        //unloading item: check chest excess
        if (this.enabUnload && this.chest != null)
            excessList[1] = InventoryHelper.checkInventoryAmount(this.chest, this.getItemstackTemp(false), this.getItemMode(false), checkMetadata, checkNbt, checkOredict, true);
    ...
    private boolean isInventoryRemain() {
        ...
        //loading item: check ship excess
        if (this.enabLoad && this.chest != null)
            remainList[0] = InventoryHelper.checkInventoryAmount(this.chest, this.getItemstackTemp(true), this.getItemMode(true), checkMetadata, checkNbt, checkOredict, false);
        else remainList[0] = true;

        //unloading item: check chest excess
        if (this.enabUnload && this.ship != null)
            remainList[1] = InventoryHelper.checkInventoryAmount(this.ship.getCapaShipInventory(), this.getItemstackTemp(false), this.getItemMode(false), checkMetadata, checkNbt, checkOredict, false);
            ...
            if (!tempitem.isEmpty()) {
                ...
                if (canMove && slotid >= 0) {
                    ...
                    if (invFrom instanceof CapaShipInventory)
                        moveitem = ((CapaShipInventory) invFrom).getStackInSlotWithPageCheck(slotid);
                    ...
                    if (moved && moveitem.getCount() <= 0) {
                        ...
                        else invFrom.setInventorySlotContents(slotid, ItemStack.EMPTY);
                    }
    ...
    private int matchTempItem(IInventory inv, ItemStack target) {
        ...
        if (!target.isEmpty()) {
            ...
            for (slotid = startid; slotid < maxSize; slotid++) {
                if (inv instanceof CapaShipInventory)
                    getitem = ((CapaShipInventory) inv).getStackInSlotWithPageCheck(slotid);
