...
public class EntityHelper {
    ...
    public static boolean checkEquipColled(int equipID, CapaTeitoku capa) {
        return capa != null && capa.getColleEquipList() != null && capa.getColleEquipList().contains(equipID);
    }
                    ...
                    try {
                        ...
                        if (updatePos && entity.getStateMinor(ID.M.NumGrudge) > 0) {
                            ...
                            if (entity.getStateMinor(ID.M.FormatType) > 0 &&
                                ...
                                if (entity instanceof BasicEntityMount) {
                                    BasicEntityShip host = (BasicEntityShip) entity.getHostEntity();
                                    if (host != null)
                                        applyMovingToShipTeam(host, host.getGuardedPos(0) + 0.5D, host.getGuardedPos(1), host.getGuardedPos(2) + 0.5D);
                                } else {
    ...
    public static <T> ArrayList getEntitiesWithinAABB(World world, Class<? extends T> cls, AxisAlignedBB aabb, @Nullable Predicate filter) {
        ...
        //check entity class
        for (Entity entity : list) {
            if (cls.isAssignableFrom(entity.getClass()) && filter.apply(entity)) {
                list2.add((T) entity);
            }
        }
    ...
    public static <T extends EntityLivingBase & IShipNavigator> void moveEntityWithHeading(T host, float strafe, float vertical, float forward) {
        ...
        //非液體中移動
            ...
            if (host.isOnLadder()) {
                float f9 = 0.15F;
                host.motionX = MathHelper.clamp(host.motionX, -f9, f9);
                host.motionZ = MathHelper.clamp(host.motionZ, -f9, f9);
        ...
        if (mount != null && cls != null) {
            ...
            if (list.size() > 0 && list.size() > id) {
                ...
                //apply id checking
                    ...
                    return cls.isAssignableFrom(list.get(id).getClass());
                }
            }//end list not empty
        }//end null check
    ...
    public static int getMoraleLevel(int m) {
        if (m > ID.Morale.L_Excited) {  //excited
