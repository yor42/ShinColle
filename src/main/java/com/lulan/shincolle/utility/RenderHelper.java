...
public class RenderHelper {
    ...
    public static void drawTexturedModalRectWithUV(float xCoord, float yCoord, int minU, int minV, int maxU, int maxV, double zLevel) {
        ...
        BufferBuilder vertexbuffer = tessellator.getBuffer();
        vertexbuffer.begin(7, DefaultVertexFormats.POSITION_TEX);
        vertexbuffer.pos(xCoord + 0.0F, yCoord + (float) maxV, zLevel).tex((float) (minU) * 0.00390625F, (float) (minV + maxV) * 0.00390625F).endVertex();
        vertexbuffer.pos(xCoord + (float) maxU, yCoord + (float) maxV, zLevel).tex((float) (minU + maxU) * 0.00390625F, (float) (minV + maxV) * 0.00390625F).endVertex();
        vertexbuffer.pos(xCoord + (float) maxU, yCoord + 0.0F, zLevel).tex((float) (minU + maxU) * 0.00390625F, (float) (minV) * 0.00390625F).endVertex();
        vertexbuffer.pos(xCoord + 0.0F, yCoord + 0.0F, zLevel).tex((float) (minU) * 0.00390625F, (float) (minV) * 0.00390625F).endVertex();
        tessellator.draw();
    }
        ...
        try {
            ...
            for (int k = 0; k < 5; k++) {
                if (drawBtn[k]) {
                    ...
                    //draw button icon
                    RenderHelper.drawTexturedModalRect(px2, py, k * 18, 0, 18, 18, 0);
                    //draw button cooldown icon
                    RenderHelper.drawTexturedModalRect(px2, py + 18 - len, 0, 36 - len, 18, len, 0);
                    //draw button cooldown text
                    if (len > 0)
                        fr.drawStringWithShadow(String.format("%.1f", (float) drawCD[k] * 0.05F), px2 + 5, py + 18, Enums.EnumColors.YELLOW.getValue());
                    //draw click effect
                    if (keySet.keyBindsHotbar[k].isKeyDown()) Gui.drawRect(px2, py, px2 + 18, py + 18, 1073741823);

                    if (k == 2)
                        fr.drawStringWithShadow(String.valueOf(drawAirNum[0]), px2 + 7, py - 8, Enums.EnumColors.GREEN.getValue());
                    else if (k == 3)
                        fr.drawStringWithShadow(String.valueOf(drawAirNum[1]), px2 + 7, py - 8, Enums.EnumColors.CYAN.getValue());
                }
            }
