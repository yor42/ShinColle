...
public class BlockHelper {
    ...
    public static boolean checkBlockSafe(IBlockState state) {
        return state == null || state.getMaterial() == Material.AIR || checkBlockIsLiquid(state) ||
    ...
    public static boolean checkBlockIsLiquid(IBlockState state) {
        return state != null && (state instanceof IFluidBlock || state instanceof BlockLiquid || state.getMaterial().isLiquid());
    }
    ...
    public static boolean checkBlockIsLiquid(IBlockState state, int level) {
        if (state != null) {
            if (state instanceof BlockLiquid) {
                return state.getValue(BlockLiquid.LEVEL).intValue() == level;
            }

            return state instanceof IFluidBlock ||
    ...
    public static boolean checkBlockCanStandAt(IBlockState block) {
        if (block != null) {
            ...
            if (mat != Material.AIR && mat != Material.FIRE) {
                return checkBlockIsLiquid(block) || block.getMaterial().blocksMovement();
            }
        }
