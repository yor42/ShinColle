...
public class ParticleHelper {
    ...
    public static void spawnAttackParticleAt(double posX, double posY, double posZ, double lookX, double lookY, double lookZ, byte type) {
        ...
        switch (type) {
            case 1:        //largeexplode
                world.spawnParticle(EnumParticleTypes.EXPLOSION_LARGE, posX, posY + 2, posZ, 0.0D, 0.0D, 0.0D);
                break;
            case 2:        //hugeexplosion
                world.spawnParticle(EnumParticleTypes.EXPLOSION_HUGE, posX, posY + 1, posZ, 0.0D, 0.0D, 0.0D);
                for (int i = 0; i < 24; ++i) {
                    ran1 = rand.nextFloat() * 6F - 3F;
                    ran2 = rand.nextFloat() * 6F - 3F;
                    world.spawnParticle(EnumParticleTypes.LAVA, posX + ran1, posY + 1, posZ + ran2, 0D, 0D, 0D);
                }
                break;
            case 3:        //hearts effect
                for (int i = 0; i < 7; ++i) {
                    ...
                    world.spawnParticle(EnumParticleTypes.HEART, posX + rand.nextFloat() * 2D - 1D, posY + 0.5D + rand.nextFloat() * 2D, posZ + rand.nextFloat() * 2.0F - 1D, d0, d1, d2);
                }
                break;
            case 4:    //smoke: for minor damage
                for (int i = 0; i < 3; i++) {
                    ...
                    world.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, posX + ran1, posY + ran2, posZ + ran3, 0D, lookY, 0D);
                }
                break;
            case 5:        //flame+smoke: for moderate damage
                for (int i = 0; i < 3; i++) {
                    ...
                    ran3 = rand.nextFloat() * lookX - lookX / 2D;
                    world.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, posX + ran1, posY + ran2, posZ + ran3, 0D, lookY, 0D);
                    world.spawnParticle(EnumParticleTypes.FLAME, posX + ran3, posY + ran2, posZ + ran1, 0D, lookY, 0D);
                }
                break;
            case 6:    //largesmoke
                for (int i = 0; i < 24; i++) {
                    ...
                    ran2 = rand.nextFloat();
                    ran3 = rand.nextFloat();
                    world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, posX + lookX - 0.5D + 0.05D * i, posY + 0.6D + ran1, posZ + lookZ - 0.5D + 0.05D * i, lookX * 0.3D * ran2, 0.05D * ran2, lookZ * 0.3D * ran2);
                    world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, posX + lookX - 0.5D + 0.05D * i, posY + 1.0D + ran1, posZ + lookZ - 0.5D + 0.05D * i, lookX * 0.3D * ran3, 0.05D * ran3, lookZ * 0.3D * ran3);
                }
                break;
            case 7:    //flame+large smoke: for heavy damage
                for (int i = 0; i < 4; i++) {
                    ...
                    ran2 = rand.nextFloat() * lookX - lookX / 2D;
                    ran3 = rand.nextFloat() * lookX - lookX / 2D;
                    world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, posX + ran1, posY + ran2, posZ + ran3, 0D, 0D, 0D);
                    world.spawnParticle(EnumParticleTypes.FLAME, posX + ran3, posY + ran2, posZ + ran1, 0D, 0.05D, 0D);
                }
                break;
            case 8:        //flame
                world.spawnParticle(EnumParticleTypes.FLAME, posX, posY - 0.1, posZ, 0.0D, 0.0D, 0.0D);
                world.spawnParticle(EnumParticleTypes.FLAME, posX, posY, posZ, 0.0D, 0.0D, 0.0D);
                world.spawnParticle(EnumParticleTypes.FLAME, posX, posY + 0.1, posZ, 0.0D, 0.0D, 0.0D);
                break;
            case 9:    //lava + largeexplode
                world.spawnParticle(EnumParticleTypes.EXPLOSION_LARGE, posX, posY + 1.5, posZ, 0.0D, 0.0D, 0.0D);
                for (int i = 0; i < 15; i++) {
            ...
            case 19:    //double largesmoke for 14~20 inch cannon
                ...
                for (int i = 0; i < 15; i++) {
                    ...
                    ran2 = rand.nextFloat();
                    ran3 = rand.nextFloat();
                    world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, posX + lookX - 0.5D + 0.05D * i + newPos1[1], posY + 0.6D + ran1, posZ + lookZ - 0.5D + 0.05D * i + newPos1[0], lookX * 0.3D * ran2, 0.05D * ran2, lookZ * 0.3D * ran2);
                    world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, posX + lookX - 0.5D + 0.05D * i + newPos2[1], posY + 0.6D + ran1, posZ + lookZ - 0.5D + 0.05D * i + newPos2[0], lookX * 0.3D * ran3, 0.05D * ran3, lookZ * 0.3D * ran3);
                    world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, posX + lookX - 0.5D + 0.05D * i + newPos1[1], posY + 0.9D + ran1, posZ + lookZ - 0.5D + 0.05D * i + newPos1[0], lookX * 0.3D * ran3, 0.05D * ran3, lookZ * 0.3D * ran3);
                    world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, posX + lookX - 0.5D + 0.05D * i + newPos2[1], posY + 0.9D + ran1, posZ + lookZ - 0.5D + 0.05D * i + newPos2[0], lookX * 0.3D * ran2, 0.05D * ran2, lookZ * 0.3D * ran2);
                }
                break;
            case 20:    //smoke: for nagato equip
                for (int i = 0; i < 3; i++) {
                    world.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, posX, posY + i * 0.1D, posZ, lookX, lookY, lookZ);
                }
                break;
            ...
            case 24:    //smoke: for nagato BOSS equip
                for (int i = 0; i < 3; i++) {
                    world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, posX, posY + i * 0.3D, posZ, lookX, lookY, lookZ);
                }
                break;
            ...
            case 30:    //snow hit
                for (int i = 0; i < 15; i++) {
                    ...
                    world.spawnParticle(EnumParticleTypes.SNOWBALL, posX + ran1, posY + 0.8D + ran2, posZ + ran3, lookX * 0.2D, 0.5D, lookZ * 0.2D);
                }
                break;
            case 31:    //throw snow smoke
                for (int i = 0; i < 22; i++) {
                    ...
                    ran2 = rand.nextFloat();
                    ran3 = rand.nextFloat();
                    world.spawnParticle(EnumParticleTypes.SNOW_SHOVEL, posX + lookX - 0.5D + 0.05D * i, posY + 0.7D + ran1, posZ + lookZ - 0.5D + 0.05D * i, lookX * 0.3D * ran2, 0.05D * ran2, lookZ * 0.3D * ran2);
                    world.spawnParticle(EnumParticleTypes.SNOW_SHOVEL, posX + lookX - 0.5D + 0.05D * i, posY + 0.9D + ran1, posZ + lookZ - 0.5D + 0.05D * i, lookX * 0.3D * ran3, 0.05D * ran3, lookZ * 0.3D * ran3);
                }
                break;
            ...
            case 35:    //triple largesmoke for boss ship
                ...
                for (int i = 0; i < 15; i++) {
                    ...
                    ran2 = rand.nextFloat();
                    ran3 = rand.nextFloat();
                    world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, posX + lookX - 0.6D + 0.1D * i + newPos1[1] + ran2, posY + ran1, posZ + lookZ - 0.6D + 0.1D * i + newPos1[0] + ran2, lookX * 0.3D * ran2, 0.05D * ran2, lookZ * 0.3D * ran2);
                    world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, posX + lookX - 0.6D + 0.1D * i + newPos2[1] + ran3, posY + ran1, posZ + lookZ - 0.6D + 0.1D * i + newPos2[0] + ran3, lookX * 0.3D * ran3, 0.05D * ran3, lookZ * 0.3D * ran3);
                    world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, posX + lookX - 0.6D + 0.1D * i + newPos1[1] + ran3, posY + 0.3D + ran1, posZ + lookZ - 0.6D + 0.1D * i + newPos1[0] + ran3, lookX * 0.3D * ran3, 0.05D * ran3, lookZ * 0.3D * ran3);
                    world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, posX + lookX - 0.6D + 0.1D * i + newPos2[1] + ran2, posY + 0.3D + ran1, posZ + lookZ - 0.6D + 0.1D * i + newPos2[0] + ran2, lookX * 0.3D * ran2, 0.05D * ran2, lookZ * 0.3D * ran2);
                    world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, posX + lookX - 0.6D + 0.1D * i + newPos1[1] + ran2, posY + 0.6D + ran1, posZ + lookZ - 0.6D + 0.1D * i + newPos1[0] + ran2, lookX * 0.3D * ran3, 0.05D * ran3, lookZ * 0.3D * ran3);
                    world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, posX + lookX - 0.6D + 0.1D * i + newPos2[1] + ran3, posY + 0.6D + ran1, posZ + lookZ - 0.6D + 0.1D * i + newPos2[0] + ran3, lookX * 0.3D * ran2, 0.05D * ran2, lookZ * 0.3D * ran2);
                }
                break;
            case 36:    //emotion
            ...
            case 48:    //water bubble
            {
                for (int i = 0; i < 14; i++) {
                    ...
                    world.spawnParticle(EnumParticleTypes.WATER_BUBBLE, posX + ran2, posY + ran1, posZ + ran3, 0D, 0D, 0D);
                    world.spawnParticle(EnumParticleTypes.WATER_WAKE, posX + ran2, posY + ran1, posZ + ran3, 0D, 0D, 0D);
                }
            }
            break;
            case 49:    //drip lava
        ...
        switch (type) {
            ...
            case 5:    //custom largesmoke: par1:wide, par2:length, par3:height, EntityLivingBase ONLY
            {
                ...
                for (int i = 0; i < 24; i++) {
                    ...
                    world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, ent.posX + newPos1[1] + ran1, ent.posY + par3 + ran2, ent.posZ + newPos1[0] + ran3, newPos3[1] * ran4, 0.05D * ran4, newPos3[0] * ran4);
                    world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, ent.posX + newPos2[1] + ran1, ent.posY + par3 + ran3, ent.posZ + newPos2[0] + ran2, newPos3[1] * ran4, 0.05D * ran4, newPos3[0] * ran4);
                    world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, ent.posX + newPos1[1] + ran2, ent.posY + par3 + ran1, ent.posZ + newPos1[0] + ran3, newPos3[1] * ran4, 0.05D * ran4, newPos3[0] * ran4);
                    world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, ent.posX + newPos2[1] + ran2, ent.posY + par3 + ran3, ent.posZ + newPos2[0] + ran1, newPos3[1] * ran4, 0.05D * ran4, newPos3[0] * ran4);
                    world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, ent.posX + newPos1[1] + ran3, ent.posY + par3 + ran1, ent.posZ + newPos1[0] + ran2, newPos3[1] * ran4, 0.05D * ran4, newPos3[0] * ran4);
                    world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, ent.posX + newPos2[1] + ran3, ent.posY + par3 + ran2, ent.posZ + newPos2[0] + ran1, newPos3[1] * ran4, 0.05D * ran4, newPos3[0] * ran4);
                }
            }
            break;
            ...
            {
                //煙霧出現位置: 依照身體旋轉
                newPos1 = CalcHelper.rotateXZByAxis((float) par3, (float) par1, ((EntityLivingBase) ent).renderYawOffset * Values.N.DIV_PI_180, 1F);
                newPos2 = CalcHelper.rotateXZByAxis((float) par3, (float) -par1, ((EntityLivingBase) ent).renderYawOffset * Values.N.DIV_PI_180, 1F);
                //煙霧噴射方向: 依照頭部旋轉
                newPos3 = CalcHelper.rotateXZByAxis(1.5F, 0F, ((EntityLivingBase) ent).rotationYawHead * Values.N.DIV_PI_180, 1F);
                ...
                for (int i = 0; i < 24; i++) {
                    ...
                    ran2 = rand.nextFloat();
                    ran3 = rand.nextFloat();
                    world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, ent.posX - 0.5D + 0.05D * i + newPos1[1] + newPos3[1], ent.posY + par2 + 0.6D + ran1, ent.posZ - 0.5D + 0.05D * i + newPos1[0] + newPos3[0], newPos3[1] * 0.5D * ran2, 0.05D * ran2, newPos3[0] * 0.5D * ran2);
                    world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, ent.posX - 0.5D + 0.05D * i + newPos2[1] + newPos3[1], ent.posY + par2 + 0.6D + ran1, ent.posZ - 0.5D + 0.05D * i + newPos2[0] + newPos3[0], newPos3[1] * 0.5D * ran3, 0.05D * ran3, newPos3[0] * 0.5D * ran3);
                    world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, ent.posX - 0.5D + 0.05D * i + newPos1[1] + newPos3[1], ent.posY + par2 + 0.9D + ran1, ent.posZ - 0.5D + 0.05D * i + newPos1[0] + newPos3[0], newPos3[1] * 0.5D * ran3, 0.05D * ran3, newPos3[0] * 0.5D * ran3);
                    world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, ent.posX - 0.5D + 0.05D * i + newPos2[1] + newPos3[1], ent.posY + par2 + 0.9D + ran1, ent.posZ - 0.5D + 0.05D * i + newPos2[0] + newPos3[0], newPos3[1] * 0.5D * ran2, 0.05D * ran2, newPos3[0] * 0.5D * ran2);
                }
            }
            break;
            ...
            {
                //											 ent, type, scale, rad, NO_USE, NO_USE, 4~7:RGBA, height
                ParticleSparkle spark1 = new ParticleSparkle(ent, 3, 0.05F, ent.width, 0F, 0F, (float) par1, (float) par2, (float) par3, 1F, ent.height * 0.4F);
                Minecraft.getMinecraft().effectRenderer.addEffect(spark1);
            }
            break;
