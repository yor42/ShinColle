...
public class InventoryHelper {
    ...
    public static boolean matchTargetItem(ItemStack target, ItemStack temp, boolean checkMetadata, boolean checkNbt, boolean checkOredict) {
        if (!temp.isEmpty() && !target.isEmpty()) {
            ...
            if (target.getItem() == temp.getItem()) {
                ...
                if (checkNbt && checkMetadata) {
                    return ItemStack.areItemStackTagsEqual(target, temp) &&
                            target.getItemDamage() == temp.getItemDamage();
                }
                //check nbt only
                else if (checkNbt) {
                    return ItemStack.areItemStackTagsEqual(target, temp);
                ...
                //check meta only
                    ...
                    return target.getItemDamage() == temp.getItemDamage();
                }
        ...
        } else return temp.isEmpty() && target.isEmpty();
    ...
    public static boolean mergeItemStack(IInventory inv, ItemStack itemstack, int[] slots) {
        ...
        if (itemstack.isStackable()) {
            ...
            while (itemstack.getCount() > 0 && k < maxSize) {
                ...
                //get stack in slot
                if (inv instanceof CapaShipInventory)
                    slotstack = ((CapaShipInventory) inv).getStackInSlotWithPageCheck(j);
        ...
        if (itemstack.getCount() > 0) {
            ...
            while (k < maxSize) {
                ...
                if (slotstack.isEmpty()) {
                    if (inv instanceof CapaShipInventory)
                        ((CapaShipInventory) inv).setInventorySlotWithPageCheck(j, itemstack.copy());
