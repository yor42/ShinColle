        ...
        for (Map.Entry<Integer, Integer> entry : buffmap.entrySet()) {
            ...
            switch (id) {
                case 10: //regen: heal X every ticking
                    if (host.getHealth() < host.getMaxHealth()) host.heal((hp1p + 4F) * (1F + (float) lv * 0.5F));
                    break;
                case 20: //wither: hurt X until dead
                    host.attackEntityFrom(DamageSource.MAGIC, (hp1p + 4F) * (1F + (float) lv * 0.5F));
                    break;
                case 23: //saturation: morale +N/lv, friendly ship only
                    if (host.getHealth() < host.getMaxHealth())
                        host.heal((hp1p + 2F) * (0.8F + (float) lv * 0.2F));
    ...
    public static float applyBuffOnHeal(EntityLivingBase host, float heal) {
        ...
        if (!getlist.isEmpty()) {
            ...
            for (EntityAreaEffectCloud aecloud : getlist) {
                ...
                if (level > 0) {
                    return heal + (hp1p + 3F) * (1F + (float) level * 0.5F);
                }
            }
        }
        ...
        if (!getlist2.isEmpty()) {
            ...
            for (EntityPotion potion : getlist2) {
                ...
                if (!pstack.isEmpty()) {
                    ...
                    //return heal value by potion level
                    if (level > 0) {
                        return heal + (hp1p + 6F) * (1F + (float) level * 0.5F);
                    }
                }
            }
        }
