        ...
        if (host != null && !((Entity) host).world.isRemote) {
            ...
            if (host instanceof IShipInvisible) {
                if (distSq > 36F) {
                    ...
                    //check limit
                    if (dodge > ConfigHandler.limitShipAttrs[ID.Attrs.DODGE])
                        dodge = (float) ConfigHandler.limitShipAttrs[ID.Attrs.DODGE];
                }
            ...
            if (rand.nextFloat() <= dodge) {
                //check host is morph
                if (ent2 instanceof IShipMorph && ((IShipMorph) ent2).getMorphHost() != null)
                    ent2 = ((IShipMorph) ent2).getMorphHost();
