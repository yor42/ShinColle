...
public class EntityAIShipGuarding extends EntityAIBase {
    ...
    private final double[] guardPosOld; // last update position
    ...
    private final int[] delayTime; // attack delay time: 0:light 1:heavy 2:aircraft
    private final int[] maxDelayTime; // attack max delay time
        ...
        if (host != null) {
            ...
            if (this.host2.dimension == this.host.getGuardedPos(3)) {
                    ...
                    if (this.checkTP_D > ConfigHandler.shipTeleport[0]) {
                        ...
                        if (host2 != null && owner != null)
                            LogHelper.debug("DEBUG: guard AI: distSQ > " + ConfigHandler.shipTeleport[1]
                                    + " , teleport to target. dim: " + host2.dimension + " " + owner.dimension);
                ...
                if (this.checkTP_T > ConfigHandler.shipTeleport[0]) {
                    ...
                    if (host2 != null && owner != null)
                        LogHelper
                                .debug("DEBUG: guard AI: teleport entity: dimension check: " + host2.dimension + " " + owner.dimension);
                    EntityHelper.applyTeleport(this.host, this.distSq, new Vec3d(pos[0], pos[1] + 0.75D, pos[2]));
                }
            } // end same dim
            else {
    ...
    private void updateAttackParms() {
        if (this.ship != null) {
            ...
            // attack delay
            this.maxDelayTime[0] = (int) (ConfigHandler.baseAttackSpeed[1] / (this.ship.getAttrs().getAttackSpeed()))
                    + ConfigHandler.fixedAttackDelay[1];
            this.maxDelayTime[1] = (int) (ConfigHandler.baseAttackSpeed[2] / (this.ship.getAttrs().getAttackSpeed()))
                    + ConfigHandler.fixedAttackDelay[2];
            this.maxDelayTime[2] = (int) (ConfigHandler.baseAttackSpeed[3] / (this.ship.getAttrs().getAttackSpeed()))
    ...
    private void findTarget() {
        AxisAlignedBB fireRange = this.host2.getEntityBoundingBox().grow(this.range * 0.9D, this.range * 0.6D,
                this.range * 0.9D);
        fireRange = fireRange.grow(this.range * -0.9D, this.range * -0.6D, this.range * -0.9D);
        List<EntityLivingBase> list1 = this.host2.world.getEntitiesWithinAABB(EntityLivingBase.class, fireRange,
                this.targetSelector);
        String str = "";
        ...
        // get nearest target
        if (list1.size() > 2) {
            this.target = list1.get(this.host2.world.rand.nextInt(3));
    ...
    private boolean checkGuardTarget() {
            ...
            if (!this.guarded.isEntityAlive() ||
                ...
                if (host.getStateMinor(ID.M.FormatType) > 0) {
                    ...
                    if (dsq > 6) {
                        ...
                        if (owner != null
                                && (ConfigHandler.alwaysShowTeamParticle || !EntityHelper.getPointerInUse(owner).isEmpty()) &&
                                owner.dimension == host2.dimension) {
                            CommonProxy.channelP.sendTo(new S2CSpawnParticle(25, pos[0], pos[1], pos[2], 0.3, 4, 0),
                    ...
                    if (this.host2.ticksExisted % 16 == 0) {
                        ...
                        if (owner != null
                                && (ConfigHandler.alwaysShowTeamParticle || !EntityHelper.getPointerInUse(owner).isEmpty()) &&
                                owner.dimension == host2.dimension) {
                            CommonProxy.channelP.sendTo(new S2CSpawnParticle(25, pos[0], pos[1], pos[2], 0.3, 6, 0),
                                    (EntityPlayerMP) owner);
                        }
                    }
                }
        ...
        if (pos[1] <= 0 || this.host2.dimension != this.host.getGuardedPos(3)) {
            ...
            return this.distSq > this.maxDistSq && host2.dimension == host.getStateMinor(ID.M.GuardDim);
        }
    }
