...
public class EntityAIShipOpenDoor extends EntityAIBase {
    ...
    private float vecX;
    private float vecZ;
    private final boolean closeDoor;        //通過門後是否要隨手關門
    ...
    private static void toggleGate(BlockPos pos, Entity host, IBlockState state, boolean openGate) {
        ...
        if (state.getBlock() instanceof BlockFenceGate) {
            //open gate
            if (state.getValue(BlockFenceGate.OPEN).booleanValue()) {
            ...
            //close gate
                ...
                {
                    EnumFacing enumfacing = EnumFacing.fromAngle(host.rotationYaw);
