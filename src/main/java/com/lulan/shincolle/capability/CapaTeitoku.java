...
public class CapaTeitoku implements ICapaTeitoku, IInventory {
    ...
    public static void setCapaDataMisc(int[] value) {
        ...
        if (capa != null) {
            ...
            capa.isRingActive = value[0] > 0;
            capa.hasTeam = value[1] > 0;
            capa.teamId = value[2];
    ...
    public boolean hasTeam() {
        ...
        if (player != null && !player.world.isRemote) {
            if (ServerProxy.getTeamData(this.playerUID) != null)
                return true;
        }
    ...
    public int getNumberOfShip(int teamID) {
        ...
        if (this.teamList != null) {
            for (int i = 0; i < 6; ++i) {
                if (this.teamList[teamID][i] != null &&
                        this.teamList[teamID][i].isEntityAlive())
                    num++;
            }
        }
    ...
    public void addShipEntity(int slot, BasicEntityShip entity, boolean forceAdd) {
        ...
        if (forceAdd) {
            ...
            if (entity != null) {
                ...
                // for (int k = 0; k < 6; k++)
                // {
                // LogHelper.info("DEBUG : team list (before add) "+this.saveId+" "+k+"
                // "+this.teamList[k]);
                // }
    ...
    public boolean isTeamAlly(int par1) {
        ...
        if (this.player.world.isRemote) {
            if (this.getPlayerTeamAllyList() != null) {
                // check team ally id
                return this.getPlayerTeamAllyList().contains(par1);
            }
        }
    ...
    public boolean isTeamBanned(int par1) {
        ...
        if (this.player.world.isRemote) {
            if (this.getPlayerTeamBannedList() != null) {
                // check team ally id
                return this.getPlayerTeamBannedList().contains(par1);
            }
        }
        ...
        if (player.world != null && !player.world.isRemote) {
            switch (type) {
                case 0:
                    ...
                    CommonProxy.channelG.sendTo(new S2CGUIPackets(this, S2CGUIPackets.PID.SyncPlayerProp),
                            (EntityPlayerMP) player);
                    break;
                case 1:
                    CommonProxy.channelG.sendTo(new S2CGUIPackets(this, S2CGUIPackets.PID.SyncPlayerProp_Formation),
                            (EntityPlayerMP) player);
                    break;
                case 2:
                    CommonProxy.channelG.sendTo(new S2CGUIPackets(this, S2CGUIPackets.PID.SyncPlayerProp_TargetClass),
                            (EntityPlayerMP) player);
                    break;
                case 3:
                    CommonProxy.channelG.sendTo(new S2CGUIPackets(this, S2CGUIPackets.PID.SyncPlayerProp_TeamData),
                            (EntityPlayerMP) player);
                    break;
                case 4:
                    CommonProxy.channelG.sendTo(new S2CGUIPackets(this, S2CGUIPackets.PID.SyncPlayerProp_ShipsAll),
                            (EntityPlayerMP) player);
                    break;
                case 5:
                    CommonProxy.channelG.sendTo(
                            new S2CGUIPackets(S2CGUIPackets.PID.SyncPlayerProp_ColledShip, this.getColleShipList()),
                            (EntityPlayerMP) player);
                    break;
                case 6:
                    CommonProxy.channelG.sendTo(
                            new S2CGUIPackets(S2CGUIPackets.PID.SyncPlayerProp_ColledEquip, this.getColleEquipList()),
                            (EntityPlayerMP) player);
                    break;
                case 7:
                    CommonProxy.channelG.sendTo(new S2CGUIPackets(this, S2CGUIPackets.PID.SyncPlayerProp_Misc),
                            (EntityPlayerMP) player);
                    break;
                case 8:
                    CommonProxy.channelG.sendTo(new S2CGUIPackets(this, S2CGUIPackets.PID.SyncPlayerProp_UnitName),
                            (EntityPlayerMP) player);
                    break;
            }
        }
    ...
    public void syncShips(int teamID) {
        if (player.world != null && !player.world.isRemote) {
            CommonProxy.channelG.sendTo(new S2CGUIPackets(this, S2CGUIPackets.PID.SyncPlayerProp_ShipsInTeam, teamID),
                    (EntityPlayerMP) player);
        }
    }
    ...
    public void swapShip(int tid, int posA, int posB) {
        ...
        if (this.teamList[tid][posA] != null)
            this.teamList[tid][posA].setUpdateFlag(ID.FlagUpdate.FormationBuff, true);
        ...
        for (int i = 0; i < this.teamList[tid].length; i++) {
            if (this.teamList[tid][i] != null)
                ships.add(this.teamList[tid][i]);
        }

        if (ships.size() > 4) {
            // update formation guard position
            FormationHelper.applyFormationMoving(ships, getFormatID(tid), (int) ships.get(0).posX, (int) ships.get(0).posY,
                    (int) ships.get(0).posZ, false);
        }
    }
