 ...
 * Fuel Cost = BaseCost + CostPerMaterial * ( TotalMaterialAmount - minAmount *
 * 4 )
 ...
 * MinMaterial / MinFuelCost = 100*4 / 460800 = BaseCost(460800)
 * CostPerMaterial(256)
 * <p>
 * <p>
...
public class LargeRecipes {
    ...
    public static boolean canRecipeBuild(int[] matAmount) {
        return matAmount[0] >= MIN_AMOUNT && matAmount[1] >= MIN_AMOUNT && matAmount[2] >= MIN_AMOUNT
    ...
    private static void addSlotContents(TileMultiGrudgeHeavy tile, Item item, int meta, int slot) {
        if (tile.getStackInSlot(slot).isEmpty()) { // 空slot, 新增itemstack
