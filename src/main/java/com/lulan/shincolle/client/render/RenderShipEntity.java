...
//        			d1 -= (1.6D - (double)host.height) * 0.5D + 0.3D;
            ...
            double d3 = this.interp(entity.prevRotationYaw, entity.rotationYaw, parTick * 0.5F) * 0.01745329238474369D;
            double d4 = this.interp(entity.prevRotationPitch, entity.rotationPitch, parTick * 0.5F) * 0.01745329238474369D;
            ...
            double d9 = this.interp(entity.prevPosX, entity.posX, parTick) - d5 * 0.7D - d6 * 0.5D * d8;
            double d10 = this.interp(entity.prevPosY + (double) entity.getEyeHeight() * 0.7D, entity.posY + (double) entity.getEyeHeight() * 0.7D, parTick) - d7 * 0.5D - 0.25D;
            double d11 = this.interp(entity.prevPosZ, entity.posZ, parTick) - d6 * 0.7D + d5 * 0.5D * d8;
            double d12 = this.interp(host.prevRenderYawOffset, host.renderYawOffset, parTick) * 0.01745329238474369D + (Math.PI / 2D);
