...
public class GuiShipInventory extends GuiContainer {
    ...
    private static final ResourceLocation TEXTURE_BG = new ResourceLocation(Tags.TEXTURES_GUI + "GuiShipInventory.png");
    ...
    public GuiShipInventory(InventoryPlayer invPlayer, BasicEntityShip entity) {
        ...
        if (this.entity instanceof EntityDestroyerInazuma ||
            ...
            if (this.entity.getRidingState() > 1) {
                shipType = ID.ShipType.HEAVY_CRUISER;
                shipClass = ID.ShipClass.Raiden;
...
//		entity.rotationYawHead = f6;
    ...
    public void initGui() {
        ...
        widthHoveringText1 = temp;
        temp = this.fontRenderer.getStringWidth(strAttrAIR);
        if (temp > widthHoveringText1) widthHoveringText1 = temp;
        ...
        widthHoveringText3 = temp;
        temp = this.fontRenderer.getStringWidth(strAttrATK);
        if (temp > widthHoveringText3) widthHoveringText3 = temp;
        temp = this.fontRenderer.getStringWidth(strAttrAIR);
        if (temp > widthHoveringText3) widthHoveringText3 = temp;
    ...
    protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
        ...
        drawTexturedModalRect(guiLeft + 62, guiTop + pageIndicator, 74, 214, 6, 34);
        ...
        drawTexturedModalRect(guiLeft + 135, guiTop + pageIndicator, 74, 214, 6, 34);
        ...
        switch (this.showPageAI) {
            ...
            case 2: {    //page 2
                ...
                //get button value
                int fMinPos = (int) (((entity.getStateMinor(ID.M.FollowMin) - 1) / 30F) * 42F);
                int fMaxPos = (int) (((entity.getStateMinor(ID.M.FollowMax) - 2) / 30F) * 42F);
                int fleeHPPos = (int) ((entity.getStateMinor(ID.M.FleeHP) / 100F) * 42F);
            ...
            case 5: {    //page 5
                ...
                //get button value
                int wpStayPos = (int) (entity.getStateMinor(ID.M.WpStay) * 0.0625F * 42F);
                int autoCRPos = (int) ((entity.getStateMinor(ID.M.UseCombatRation) - 1) * 14F);
        ...
        //draw AI page indicator
        drawTexturedModalRect(guiLeft + pageIndicator, guiTop + this.pageIndicatorAI, 74, 214, 6, 11);
    ...
    private void handleHoveringText() {
        ...
        if (xMouse >= 238 + guiLeft && xMouse < 251 + guiLeft && yMouse >= 17 + guiTop && yMouse < 30 + guiTop) {
            mouseoverList.add(strMorale[EntityHelper.getMoraleLevel(this.entity.getMorale())]);
            ...
            this.drawHoveringText(mouseoverList, 126 + widthHoveringText3, 46, this.fontRenderer);
        }
        //HP tooltip
            ...
            if (showPage == 2) {
                ...
                if (yMouse >= 18 + guiTop && yMouse < 41 + guiTop) {
                    ...
                    if (this.entity instanceof IShipInvisible) {
                        ...
                        if (temp2 > ConfigHandler.limitShipAttrs[ID.Attrs.DODGE])
                            temp2 = (float) ConfigHandler.limitShipAttrs[ID.Attrs.DODGE];
                ...
                if (yMouse >= 40 + guiTop && yMouse < 62 + guiTop && this.entity.getStateMinor(ID.M.FormatType) >= 1) {
                    ...
                    this.drawHoveringText(mouseoverList, 134 + widthHoveringText3, 35, this.fontRenderer);
                }//end formation
            }//end page 3
        }
        //AI tooltip
    ...
    private void drawAttributes() {
        //draw hp, level
        String shiplevel = String.valueOf(entity.getStateMinor(ID.M.ShipLevel));
        int hpCurrent = MathHelper.ceil(entity.getHealth());
        int hpMax = MathHelper.ceil(entity.getMaxHealth());
        int color = 0;
        ...
        switch (this.showPage) {
            case 2: {    //page 2: attribute page
                String strLATK = String.format("%.1f", attrs.getAttrsBuffed(ID.Attrs.ATK_L));
                String strHATK = String.format("%.1f", attrs.getAttrsBuffed(ID.Attrs.ATK_H));
                strATK = strLATK + " / " + strHATK;
                String strALATK = String.format("%.1f", attrs.getAttrsBuffed(ID.Attrs.ATK_AL));
                String strAHATK = String.format("%.1f", attrs.getAttrsBuffed(ID.Attrs.ATK_AH));
                strAATK = strALATK + " / " + strAHATK;
                String strDEF = String.format("%.1f", attrs.getAttrsBuffed(ID.Attrs.DEF) * 100F) + "%";
                String strSPD = String.format("%.2f", attrs.getAttrsBuffed(ID.Attrs.SPD));
                String strMOV = String.format("%.2f", attrs.getAttrsBuffed(ID.Attrs.MOV));
                String strHIT = String.format("%.1f", attrs.getAttrsBuffed(ID.Attrs.HIT));
            ...
            case 1: {    //page 1: exp, kills, L&H ammo, fuel
                ...
                String grudge = String.valueOf(this.entity.getStateMinor(ID.M.NumGrudge));
        ...
        switch (this.showPageAI) {
            ...
            case 2: {    //AI page 2
                ...
                //draw value
                String followMinValue = String.valueOf(entity.getStateMinor(ID.M.FollowMin));
                String followMaxValue = String.valueOf(entity.getStateMinor(ID.M.FollowMax));
                String fleeHPValue = String.valueOf(entity.getStateMinor(ID.M.FleeHP));
            ...
            case 5:        //AI page 5
            {
                ...
                //draw value
                String strWpStayValue = CalcHelper.tick2SecOrMin(BasicEntityShip.wpStayTime2Ticks(entity.getStateMinor(ID.M.WpStay)));
                int temp = entity.getStateMinor(ID.M.UseCombatRation);

                if (this.mousePressBar == 3) {
                    barPosValue = CalcHelper.tick2SecOrMin(BasicEntityShip.wpStayTime2Ticks((int) (barPos / (42F * 0.0625F))));
                ...
                if (this.mousePressBar == 4) {
                    ...
                    if (temp2 >= 0 && temp2 <= 3) {
                        barPosValue = strMorale[temp2 + 1];
                        this.fontRenderer.drawStringWithShadow(barPosValue, 174, 169, Enums.EnumColors.RED_LIGHT.getValue());
                    }
                } else {
                    if (temp >= 0 && temp <= 4) {
                        this.fontRenderer.drawStringWithShadow(strMorale[temp], 174, 169, Enums.EnumColors.YELLOW.getValue());
                    }
                }
            }
            break;
